...
// edited by Gerardo Ardianta
// using AdaptiveBufferRouter as base class

public class ProphetRouter extends AdaptiveBufferRouter {
	...
	protected class TupleComparator implements Comparator<Tuple<Message, Connection>> {
		...
		public int compare(Tuple<Message, Connection> tuple1,
			...
			double p1 = ((ProphetRouter) tuple1.getValue().getOtherNode(getHost()).getRouter())
	...
	protected class TupleComparator2 implements Comparator<Tuple<Message, Connection>> {
		...
		public int compare(Tuple<Message, Connection> tuple1,
			...
			double pSender1 = getPredFor(dest1);
			double pSender2 = getPredFor(dest2);

			double pReceiver1 = ((ProphetRouter) tuple1.getValue().getOtherNode(getHost()).getRouter())
					.getPredFor(dest1);
			double pReceiver2 = ((ProphetRouter) tuple2.getValue().getOtherNode(getHost()).getRouter())
					.getPredFor(dest2);

			double delta1 = pReceiver1 - pSender1;
			double delta2 = pReceiver2 - pSender2;

			if (delta2 - delta1 == 0) {
				return compareByQueueMode(tuple1.getKey(), tuple2.getKey());
			} else if (delta2 - delta1 < 0) {
				return -1;
			} else {
				return 1;
			}
	...
	// common GRTR
	protected class TupleComparator3 implements Comparator<Tuple<Message, Connection>> {
